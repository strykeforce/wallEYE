...
class Processor:
    ...
    BAD_POSE = wpi.Pose3d(wpi.Translation3d(
        2767, 2767, 2767), wpi.Rotation3d())
    MIN_TAG = 1
    MAX_TAG = 16
    ...
    def __init__(self, tag_length: float):
        ...
        for i, j in enumerate(Processor.CORNER_DRAW_ORDER):
    ...
    def add_corners(tag_pos: wpi.Pose3d, corner_pos):
        return Processor.wpilib_to_cv2_coords(
    ...
    def image_pose(
        ...
        for img_index, img in enumerate(images):
            ...
            if len(corners) > 0:
                ...
                mask = (ids >= Processor.MIN_TAG) & (ids <= Processor.MAX_TAG)
                ids = ids[mask]
                corners = corners[mask]  # FIXME Unnecessary???
                # FIXME Squeeze corners, ids?

                if not mask.all():
                    Processor.logger.warning(f"Invalid Tags: {ids[~mask]}")
                ...
                for tag_count, tag_id in enumerate(ids):
                    ...
                    tags_visible_img.append(tag_id[0])

                    # Grab tag pose and calcualte each corner location
                    pose = Processor.make_pose_object(
                        layout[int(tag_id - 1)]["pose"])

                    corner_poses = np.empty(4)

                    for tag_id, j in enumerate(Processor.CORNER_POSE_ORDER):
                        corner_poses[tag_id] = Processor.wpilib_to_cv2_coords(
                            pose
                            + wpi.Transform3d(
                                wpi.Translation3d(
                                    0, *(j * self.tag_corner[:2])
                                ),
                                wpi.Rotation3d(),
                            )
                        )
                ...
                # Multi-tag
                    ...
                    try:
                        ...
                        Processor.logger.error(
                            f"solvePnP error?! Details: {e}")
                        pose1 = pose2 = Processor.BAD_POSE
                        tags_visible_img = []
                        ambig = 2767
